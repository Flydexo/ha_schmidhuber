...
class FullEpisodicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        episode = (
            self._get_table().search().where(f"episode_id = {idx}").limit(1).to_arrow()
        )
        ...
        obs_list = episode.column("observations")[
            0
        ].values  # FixedSizeListArray (T, img*img*3)
        act_list = episode.column("actions")[0].values  # FixedSizeListArray (T, 3)
        frames_np = obs_list.values.to_numpy(zero_copy_only=False).reshape(
            len(obs_list), self.img_size, self.img_size, 3
        )
        actions_np = act_list.values.to_numpy(zero_copy_only=False).reshape(
            len(act_list), -1
        )
...
def main(cfg: DictConfig) -> None:
    ...
    vae.load_state_dict(
        torch.load(
            os.path.join(HERE, cfg.rnn.vae_path), map_location=device, weights_only=True
        )
    )
    vae.eval()
    ...
    pre_kwargs = dict(
        ...
        collate_fn=collate_episode_list,
        num_workers=num_workers_pre,
        persistent_workers=num_workers_pre > 0,
    ...
    with torch.no_grad():
        for episodes in tqdm(DataLoader(ds, **pre_kwargs), desc="Pre-computing z"):
            ...
            obs_cat = torch.cat([obs[:T] for (obs, _), T in zip(episodes, lens)]).to(
                device, non_blocking=True
            )
            # uint8 HWC -> float CHW in [0,1] on the GPU (cheap there; 4x less PCIe traffic)
            obs_cat = obs_cat.permute(0, 3, 1, 2).float().div_(255)
            z_cat, _, _ = vae.encode(
                obs_cat
            )  # one forward pass for the whole batch of episodes
            z_cat = z_cat.cpu()
            offset = 0
    ...
    print(
        f"Pre-computed {len(z_cache)} episodes -- {len(train_z_ds)} train / {len(val_z_ds)} val"
    )
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, cfg.training.rnn.nb_epochs
    )
