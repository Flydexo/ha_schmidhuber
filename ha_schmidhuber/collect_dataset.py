...
def main(cfg: DictConfig) -> None:
    ...
    schema = pa.schema(
        [
            ...
            pa.field(
                "observations", pa.list_(pa.list_(pa.uint8(), IMG_SIZE * IMG_SIZE * 3))
            ),
    ...
    if USE_ENVPOOL:
        ...
        print(
            "envpool not installed (Linux x86_64 only); falling back to gym.make_vec "
    ...
    def flush_to_db(buffer, tbl, schema, start_id):
        ...
        tbl.add(
            pa.Table.from_arrays(
                [
                    ids,
                    pa.ListArray.from_arrays(offsets, flat_obs),
                    pa.ListArray.from_arrays(offsets, flat_act),
                ],
                schema=schema,
            )
        )
    ...
    while total_collected < cfg.dataset.episodes:
        ...
        for i in range(NUM_ENVS):
            ...
            env_acts[i].append(
                held_action[i].copy()
            )  # copy: held_action mutates in place
            current_obs[i] = resized[i]
            ...
            if done[i]:
                ...
                skip[
                    ...
                    not USE_ENVPOOL
                )  # gym.make_vec needs a phantom reset step; EnvPool doesn't
